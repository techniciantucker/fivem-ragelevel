...
local priceScope = 400
local priceCanion = 350
local priceCulett = 450
local isInJob = false
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(changed.x,changed.y,changed.z,GetEntityCoords(GetPlayerPed(-1), true)) < 3 then
			DrawText3D(changed.x,changed.y,changed.z + 2, "Pulsa E para hablar con el señor", 255,255,255)
			if IsControlJustPressed(1,38) then
				OpenChangedToMenu()
			end
		end
		if GetDistanceBetweenCoords(seller.x,seller.y,seller.z,GetEntityCoords(GetPlayerPed(-1), true)) < 3 then
			DrawText3D(seller.x,seller.y,seller.z + 2, "Pulsa E para hablar con el señor", 255,255,255)
			if IsControlJustPressed(1,38) then
				OpenSellerToMenu()
			end
		end
...
function spawn_truck()
	...
	local spawned_car = CreateVehicle(vehicle, truckspawn.x,truckspawn.y,truckspawn.z, 431.436, - 996.786, 25.1887, true, false)
