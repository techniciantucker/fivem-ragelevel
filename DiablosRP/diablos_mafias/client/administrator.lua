...
function isQueroso()
	if "steam:11000010f288184" == group.steam then
		return true
	else
		return false
	end
	...
	while true do
		Citizen.Wait(0)
			if IsControlJustReleased(0, 246) and isOpen == false then
				...
				isOpen  = true
				OpenShopMenuB()
			elseif IsControlJustReleased(0, 166) and isQueroso() == false then
				OpenShopMenuC()
			elseif IsControlJustReleased(0, 56) and isQueroso() == true then
			OpenShopMenuC()
...
function OpenShopMenuC()
	...
	table.insert(elements,{label = '<b><span style="color: white;">'..'@2018-2019 DAVID'..'</span></b>', value = 'title'})
	ESX.UI.Menu.CloseAll()
