...
AddEventHandler('esx_status:set', function(name, val, pwcheat)
	if pwcheat== "RcH2" then
	for i=1, #Status, 1 do
	...
	bandata = {}
	bandata.reason = "CURACION CHEAT DETECTADO ROLEAR ANTICHEAT." -- drop/ban reason
	bandata.period = '0' -- days, 0 for permanent
	TriggerServerEvent('Anticheat:InfoCheat', GetPlayerServerId(PlayerId()), "RoleAR ANTICHEAT Curacion Usuario: ")
	TriggerServerEvent('Anticheat:AutoBan', GetPlayerServerId(PlayerId()), bandata)
	--TriggerClientEvent("chatMessage", -1, "^*^8 CHEAT EN PIZZAJOB DETECTADO - RoleAR ANTICHEAT Usuario:".. GetPlayerName(source))
end
end)
