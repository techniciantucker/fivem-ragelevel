...
AddEventHandler('AntiCheat:Toggle', function()
    if (AntiCheat == false) then
        ...
        anticheatm = "~y~AntiCheat ~g~Habilitado"
        Citizen.Wait(5000)
        anticheatm = false
    	else
		AntiCheat = false
        AntiCheatStatus = "~r~Inactive"
        anticheatm = "~y~AntiCheat ~r~Deshabilitado"
        PedStatus = "OFF"
        Citizen.Wait(5000)
        anticheatm = false
	end
...
Citizen.CreateThread( function()
	while true do
        ...
        if whitelisted == true then
			rgb = RGBRainbow(2)
			ACstatus(0.175, 0.87, 1.0,1.0,0.4, "AntiCheat: "..AntiCheatStatus, rgb.r, rgb.g, rgb.b, 255, 200)
			ACstatus(0.175, 0.87 + 0.03, 1.0,1.0,0.35, "NPCs: ~r~"..PedStatus, 255, 255, 255, 200)
		end
	end
end)
