...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_carshop:getCommercialVehicles', function (vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_carshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			--print ("vehiculo: " .. Vehicles[i].model)
			--if vehiclesByCategory[Vehicles[i].category]==nil then
			--	print ("Error vehiculo: " .. Vehicles[i].model)
			--else
			table.insert(vehiclesByCategory[Vehicles[i].category], Vehicles[i])
			--end
		else
			print(('esx_carshop: vehiculo "%s" no existe'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_carshop:buyVehicleSociety', function(hasEnoughMoney)
								...
								ESX.TriggerServerCallback('esx_carshop:buyVehicleSociety', function (hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											vehicleProps.plate = newPlate
											SetVehicleNumberPlateText(vehicle, newPlate)
											TriggerServerEvent('esx_carshop:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
											ESX.ShowNotification(_U('vehicle_purchased'))
										end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('esx_carshop:sellVehicle', model)
				TriggerServerEvent('esx_carshop:addToList', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('esx_carshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('esx_carshop:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						TriggerServerEvent('esx_carshop:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))
						...
						TriggerServerEvent('esx_carshop:setVehicleForAllPlayers', vehicleProps, Config.Zones.ShopInside.Pos.x, Config.Zones.ShopInside.Pos.y, Config.Zones.ShopInside.Pos.z, 5.0)
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_carshop:getRentedVehicles', function (vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		elseif data.current.value == 'sold_vehicles' then
			...
			ESX.TriggerServerCallback('esx_carshop:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_carshop:getStockItems', function (items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_carshop:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_carshop:getPlayerInventory', function (inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_carshop:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('esx_carshop:giveBackVehicle', function(isRentedVehicle)
