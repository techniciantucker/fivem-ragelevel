...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
      ...
      {label = 'Chaleco Antibalas',		value = 'chaleco'}
		}
	}, function(data, menu)
        ...
        {label = 'Desesposar',		value = 'handcuff2'},
    ...
    local playerPed = GetPlayerPed(-1)
          SetPlayerMaxArmour(-1,900)
          SetEntityMaxHealth(-1,400)
          SetEntityHealth(-1,400)

  	SetPedComponentVariation(playerPed, 9, 27, 9, 2)
  	AddArmourToPed(playerPed, 900)
  	SetPedArmour(playerPed, 900)
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_police', _U('fine_total', label), amount, 'r0le4rh4ck', 'Policia')
			else
...
function JailPlayer(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'jail_menu', {
		...
		if jailTime == nil then
			...
			TriggerServerEvent("esx_jail:sendToJail", player, jailTime * 60,'R0le4r')
			menu.close()
		end
	end, function (data2, menu)
		menu.close()
...
AddEventHandler('esx_policejob:hasEnteredMarker', function(station, part, partNum)
  ...
  elseif part == 'SubBossActions' then
	...
	while true do
        ...
        if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'chief' then
          for i=1, #v.BossActions, 1 do
            ...
            if distance < Config.MarkerSize.x then
              isInMarker, currentStation, currentPart, currentPartNum = true, k, 'SubBossActions', i
            end
          end
        end
...
AddEventHandler('esx_policejob:getarrested', function(playerheading, playercoords, playerlocation)
	...
	SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
	local x, y, z   = table.unpack(playercoords + playerlocation * 0.5)
