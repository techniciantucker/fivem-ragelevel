...
function UpdatePlayerTable(connectedPlayers)
	...
	for k,v in pairs(connectedPlayers) do
		...
		if num == 1 then
			table.insert(formattedPlayerList, ('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td>'):format(v.name, v.id, v.trabajo , v.ping))
			num = 2
		elseif num == 2 then
			table.insert(formattedPlayerList, ('<td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>'):format(v.name, v.id, v.trabajo , v.ping))
			num = 1
		end
		...
		elseif v.job == 'mechanic' then
			mechanic = mechanic + 1
		elseif v.job == 'banksecurity' then
