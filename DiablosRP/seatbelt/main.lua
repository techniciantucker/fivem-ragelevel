...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      ...
      if IsControlJustReleased(0, 182) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "Te pusiste el cinturon", type = "success", timeout = 1400, layout = "centerLeft"})
		  	--- Täältä voit muuttaa ääntä, kun turvavyö laitetaan päälle -- Tässä ----
    		---- Here you can change sounds, when you put seatbelt on ----- Here -----
		  TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0.1, 'buckle', 0.1)
		  ...
		  TriggerEvent("pNotify:SendNotification", {text = "Te quitaste el cinturon", type = "error", timeout = 1400, layout = "centerLeft"})
		      	--- Täältä voit muuttaa ääntä, kun turvavyö otetaan pois -- Tässä ------
    			---- Here you can change sounds, when you take seatbelt off - Here -----
		  TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0.4, 'unbuckle', 0.4)
