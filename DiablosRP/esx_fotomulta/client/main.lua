...
local blips = {
	-- 60KM/H ZONES
	{title="Fotomulta (60KM/H)", colour=1, id=1, x = -524.2645, y = -1776.3569, z = 21.3384}, -- 60KM/H ZONA

	-- 80KM/H ZONA
	{title="Fotomulta (80KM/H)", colour=1, id=1, x = 2506.0671, y = 4145.2431, z = 38.1054}, -- 80KM/H ZONA

	-- 120KM/H ZONA
	{title="Fotomulta (120KM/H)", colour=1, id=1, x = 1584.9281, y = -993.4557, z = 59.3923}, -- 120KM/H ZONA
	{title="Fotomulta (120KM/H)", colour=1, id=1, x = 2442.2006, y = -134.6004, z = 88.7765}, -- 120KM/H zona
	{title="Fotomulta (120KM/H)", colour=1, id=1, x = 2871.7951, y = 3540.5795, z = 53.0930} -- 120KM/H ZONA
}
...
local Speedcamera60Zone = {
    {x = -524.2645,y = -852.6569,z = 21.3384},
    {x = 270.0645,y = -852.6569,z = 28.7884},
    {x = 192.9645,y = -835.3569,z = 28.7884}
    ...
    while true do
        ...
        for k in pairs(Speedcamera60Zone) do
            ...
            if dist <= 20.0 then
				...
				local maxSpeed = 110.0 -- Velocidad Maxima KM/H
					...
					if IsPedInAnyVehicle(playerPed, false) then
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then
							if hasBeenCaught == false then
								...
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "ghispo2" then -- BLACKLISTED VEHICLE
                ...
                elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "POLICE" then -- BLACKLISTED VEHICLE
                elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "policer8" then -- BLACKLISTED VEHICLE
                elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "POLICER8" then -- BLACKLISTED VEHICLE
                elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "polmav" then -- BLACKLISTED VEHICLE
								-- Autos en la blacklist
								else
									-- ALERT POLICE (START)
									if alertPolice == true then
										if SpeedKM > alertSpeed then
											local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))
											TriggerServerEvent('esx_phone:send', 'police', ' Alguien paso por una camara de transito a mas de ' .. alertSpeed.. ' KMH', true, {x =x, y =y, z =z})
										end
									end
									-- ALERT POLICE (END)
									...
									if useCameraSound == true then
										TriggerServerEvent("InteractSound_SV:PlayOnSource", "fotomulta", 0.5)
									end
									...
									TriggerEvent("pNotify:SendNotification", {text = "Una camara de transito te saco una fotomulta en una zona de 60kmh!", type = "error", timeout = 5000, layout = "centerLeft"})

									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Fotomulta (60KM/H)', 1500,  'Fotomulta') -- envia la factura
									else
										TriggerServerEvent('esx_speedcamera:PayBill60Zone')
									end

									hasBeenCaught = true
									Citizen.Wait(5000) --Tiempo de espera para no hacer multiples multas
								end
							end
						end
					end
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									TriggerEvent("pNotify:SendNotification", {text = "Una camara de transito te saco una foto multa en una zona de 80 kmh!", type = "error", timeout = 5000, layout = "centerLeft"})
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									TriggerEvent("pNotify:SendNotification", {text = "Una camara de transito te saco una foto multa en una zona de 120kmh", type = "error", timeout = 5000, layout = "centerLeft"})
