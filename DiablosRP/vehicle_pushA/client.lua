...
Config = {} 
Config.DamageNeeded = 100.0 -- 0.0 being broken and 1000.0 being fixed a lower value than 100.0 will break it
    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Presiona [~g~SHIFT~w~] y [~g~E~w~] para empujar el vehiculo', 0.4)
                end
