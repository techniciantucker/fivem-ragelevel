    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar activado." )
            else 
                Notify( "~b~Radar desactivado." )
            end 
            ...
            Notify( "~r~Debes de estar en un coche de policía." )
        end 
    else 
        Notify( "~r~Debes de estar en un vehículo." )
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Tipo de velocidad seteado a Km/h." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Tipo de velocidad seteado a MPH." )
    end
end 
