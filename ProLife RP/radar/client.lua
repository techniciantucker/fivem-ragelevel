...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(radarActivated == true) then
			if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) or EsCocheValido()) then
		...
		--	DrawMarker(1,plyCoords["x"]+xm-xp, plyCoords["y"]+ym-yp, plyCoords["z"]-1.0001, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
				...
				local veh = GetClosestVehicle(x, y, z, 5.001, 0, 70)
				local kmh = 0
					...
					if(GetVehicleNumberPlateText(veh) ~=nil) then
						if(GetVehicleNumberPlateText(veh)==lastNumberPlate) then
							...
							if(kmh > limitSpeed) then
								str = str.."~g~"..text[lang].speed.." : ~r~"..round(kmh,0).." KM/H\n"
							else
								str = str.."~g~"..text[lang].speed.." : ~b~"..round(kmh,0).." KM/H\n"
							end
...
function EsCocheValido()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
    local coches = {"159ATGC","FEVEREST","LROVER","MER01","MER02","OASTRA","OPOLI","POLGTR","RT1200","SEF_PFINDER","SKODA","YPOLI"}
	for i = 1,#coches,1 do 
        if GetHashKey(coches[i]) == vehicle then
            break
            return true
		end
	end
...
function tick()
	...
	if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
		if(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0)) ~= GetHashKey("POLMAV")) then
			...
			if(IsControlPressed(1,36) and IsControlPressed(1,246)) then ----- A MODIFIER !
				Wait(500)
