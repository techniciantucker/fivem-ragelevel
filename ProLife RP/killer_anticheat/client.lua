...
function checkCar(car)
	if car then
		...
		if isCarBlacklisted(carModel) then
            _DeleteEntity(car)
			TriggerServerEvent("killer_anticheat:Cars", blacklistedCar )
        end
	end
end
	...
	while true do
		...
		if IsPedJumping(PlayerPedId()) then
			...
			if jumplength > 250 then
                TriggerServerEvent("killer_anticheat:Jump", jumplength )
			end
        end
    end
	end
...
RegisterNetEvent("killer_anticheat:Toggle")
AddEventHandler("killer_anticheat:Toggle", function()
