...
function OpenShopMenuC()
	...
	if not(group.name == "") then
		table.insert(elements,{label = '<b><span style="color: yellow;">'..group.name..'</span></b>', value = 'title'})
	else
		table.insert(elements,{label = '<b><span style="color: yellow;">'..administratorLocal.noGroupSelected..'</span></b>', value = 'title'})
	end

	if not(group.money == nil) then
		table.insert(elements,{label = '<span style="color: red;">DINERO : '..group.money..'</span>', value = 'money'})
	end
	...
	if not(group.aprove == nil) then
		table.insert(elements,{label = '<span style="color: green;">PRESTIGIO : '..grouplabel[group.aprove]..'</span>', value = 'aprove'})
	end
