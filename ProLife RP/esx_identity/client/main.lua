...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Create tu primer personaje para jugar' } })
	end
end)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "dateofbirth" then
			if value == "invalid" then
				reason = "Fecha inválida de cumpleaños!"
				break
			end
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Tu personaje es demasiado obeso!"
					break
				end
			else
				reason = "Peso del personaje inválido!"
				break
			end
		end
	end
