...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if IsControlPressed(2, Keys["CAPS"]) and GetLastInputMethod(2) and Busy == false and PlayerData.job.name ~= nil and PlayerData.job.name == "police" then
                TriggerServerEvent("esx_walkie:playSoundWithinDistanceServer", 10, "copradiooff", 0.3) 
        ...
        elseif not IsControlPressed(2, Keys["CAPS"]) and GetLastInputMethod(2) and Busy == true and PlayerData.job.name ~= nil and PlayerData.job.name == "police" then
...
AddEventHandler("esx_walkie:stopAnim", function(player)
    ...
    TriggerServerEvent("esx_walkie:playSoundWithinDistanceServer",10.0, 'copradio', 0.5)
end)
