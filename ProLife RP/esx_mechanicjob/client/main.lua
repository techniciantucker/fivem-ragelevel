...
function OpenMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('tow_truck'), value = 'gruamapfre'}
				}
		...
		elseif data.current.value == 'cloakroom' then
			...
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				if skin.sex == 0 then
					...
					TriggerEvent('skinchanger:change', 'torso_1', 66)
					TriggerEvent('skinchanger:change', 'torso_2', 2)
					TriggerEvent('skinchanger:change', 'arms', 1)
					TriggerEvent('skinchanger:change', 'pants_1', 39)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBostonisMenu', 'mechanic', function(data, menu)
				menu.close()
			end)
		end
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_mechanicjob:startonitCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_mechanicjob:startonitCraft2')
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = _U('chaleco'),       value = 'chaleco'},
			{label = _U('nochaleco'),       value = 'nochaleco'}
		}
	}, function(data, menu)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_bitonilling:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
	...
	elseif data.current.value == 'nochaleco' then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicletoniInGarage', 'mechanic', vehicleProps)
