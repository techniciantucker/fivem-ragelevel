    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Has salido de la tienda, tu cesta ahora está vacía!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("No tienes nada en la cesta!", 'error', 1500)
        end
    else
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("No tienes dinero suficiente!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Puesto " .. data.current.value .. " de " .. data.current["realLabel"] .. " en la cesta ( [L] para abrirla )", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        ...
        table.insert(elements, {label = '<span style="color:red">Vaciar', value = "empty"})
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Quitado todo de la cesta.", "error", 2500)
                end
                    ...
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'basket_detailedmenu',
                        {
                            ...
                            elements = {
                                ...
                                {label = '<span style="color:red">Quitar</span>', value = "deleteItem"},
                            },
                        },
                        function(data2, menu2)
