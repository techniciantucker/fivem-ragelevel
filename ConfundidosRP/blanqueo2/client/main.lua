			...
			if data.current.value == 'wash_money' then
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_', {
					...
					if amount == nil then
						...
						menu.close()
						TriggerServerEvent('esx_moneywash2:washMoney', amount)
					end
...
--Enter / Exit Marker
AddEventHandler('esx_moneywash2:hasEnteredMarker', function(zone)
	CurrentAction     = 'wash_menu'
	CurrentActionMsg  = _U('press_menu')
	CurrentActionData = {zone = zone}
end)

AddEventHandler('esx_moneywash2:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()		
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_moneywash2:hasEnteredMarker', currentZone)
		end
		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_moneywash2:hasExitedMarker', LastZone)
		end
	end
end)
