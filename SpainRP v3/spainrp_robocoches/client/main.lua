...
Citizen.CreateThread(function()
	...
	ped = CreatePed("PED_TYPE_CIVMALE", hash, 765.97, -3191.42, 5.04, 266.946, false, true)
	SetBlockingOfNonTemporaryEvents(ped, true)
	FreezeEntityPosition(ped, true)
	SetEntityInvincible(ped,true)
	loadDict('amb@world_human_aa_smoke@male@idle_a')
    TaskPlayAnim(ped, "amb@world_human_aa_smoke@male@idle_a", "idle_a", 8.0, -8.0, -1, 1, 0, false, false, false)
				...
				if isActive == 0 then
					ESX.TriggerServerCallback('esx_carthief:anycops', function(anycops)
						if anycops >= Config.CopsRequired then
							...
							ESX.ShowAdvancedNotification('Desconocido', 'Car dealer', 'Lleva este coche a esta dirección y recibiras el pago cuando lleges', 'CHAR_HUMANDEFAULT', 1)
