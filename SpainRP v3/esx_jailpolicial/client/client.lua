...
function JailLogin()
	...
	ESX.ShowNotification("La ultima vez que te fuiste a dormir, estabas en la carcel. Te he traido de vuelta :)")
...
function UnJail()
	...
	ESX.ShowNotification("Has cumplido tu condena. Vuelve a las calles pero con calma bro")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Has sido encarcelado " .. jailTime .. " minutos")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Ya has cogido ese paquete")
							end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Encarcelar a la persona mas cercana", value = "jail_closest_player" },
				{ label = "Desencarcelar a la persona mas cercana", value = "unjail_player" }
			}
		}, 
	function(data, menu)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.UI.Menu.Open(
							...
							{
							  title = "Razon de encarcelamiento"
							},
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Rellena esto")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("La carcel esta vacia!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Prisionero: " .. playerArray[i].name .. " | Tiempo en carcel: " .. playerArray[i].jailTime .. " minutos", value = playerArray[i].identifier })
				end
