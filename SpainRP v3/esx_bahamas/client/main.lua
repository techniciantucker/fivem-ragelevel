...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job2 ~= nil and PlayerData.job2.name == 'bahamas' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_srpfacturas:sendBill', GetPlayerServerId(player), 'society_bahamas', _U('billing'), amount)
        end
...
--[[     if IsControlJustReleased(0,  Keys['F5']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'bahamas_actions') then
