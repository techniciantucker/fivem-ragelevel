...
function OpenAccessoryMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'set_unset_accessory',
	{
		...
		elements = {
			...
			{label = ('Poner ropa'), value = 'ubie'},
			{label = ('Poner/Quitar Casco'), value = 'Helmet'},
			{label = ('Poner/Quitar Gafas'), value = 'Glasses'},
			{label = ('Quitar Camiseta'), value = 'tul'},
			{label = ('Quitar Pantalones'), value = 'spo'},
			{label = ('Quitar Zapatillas'), value = 'but'}
		}
	}, function(data, menu)
		menu.close()
	if data.current.value == 'Mask' then
		SetUnsetAccessory(data.current.value)
	elseif data.current.value == 'ubie' then			
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, 194) and ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'set_unset_accessory') then
			ESX.UI.Menu.CloseAll()
		end
