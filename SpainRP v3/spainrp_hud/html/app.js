...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				if (config.showSocietyMoney == true) { document.querySelector('#society2').style.display = 'block'; }
				...
				if (config.showJob == true) { document.querySelector('#job2').style.display = 'block'; }
			...
			setStatus: function(data) {
				...
				if (data.isdead == true) {
					...
					for (i = 0; i < data.status.length; i++) {
						if ((data.status[i].name == 'hunger') || (data.status[i].name == 'thirst') || (data.status[i].name == 'stress')) { var statusValue = Math.floor(100 - data.status[i].value); }
