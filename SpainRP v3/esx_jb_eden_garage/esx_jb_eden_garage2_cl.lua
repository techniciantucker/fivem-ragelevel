...
function OpenMenuGarage(garage, KindOfVehicle, garage_name, vehicle_type)
	...
	local elements = {
		{label = "Recuperar el vehículo ("..Config.Price.."€)", value = 'return_vehicle'},
	}
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if v.fourrieremecano then
					vehicleLabel = vehicleName..': En el Deposito'
				elseif v.state then
					vehicleLabel = vehicleName..': En Garaje'.." ("..v.garage_name..")"
				else
					vehicleLabel = vehicleName..': Fuera'.." ("..v.garage_name..")"
				end
			...
			table.insert(elements, {label = "No hay coches en el garaje"})
		end
		ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_menu', {
				...
				align    = 'top-left',
				elements = {
					{label ="Sacar vehiculo" , value = 'get_vehicle_out'},
					{label ="Renombrar el vehiculo" , value = 'rename_vehicle'}
					...
					if data2.current.value == "get_vehicle_out" then
							...
							if (doesVehicleExist) then
								TriggerEvent('esx:showNotification', 'El vehiculo esta en otro garaje, ve a por el')
							elseif (data.current.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Tu vehiculo esta en el deposito, ve a comisaria')
                        elseif garage_name ~= data.current.garage_name then
							local elem = {}
							table.insert(elem, {label ="Si €"..tostring(Config.SwitchGaragePrice) , value = 'transfer_yes'})
							table.insert(elem, {label ="No" , value = 'transfer_no'})
							ESX.UI.Menu.Open(
								'default', GetCurrentResourceName(), 'transfer_menu',
								{
									title    =  "¿Quieres transferir: "..data.current.vehicleName.." al garaje actual?",
									align    = 'top-left',
									elements = elem,
								},
					...
					elseif data2.current.value == "rename_vehicle" then
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'rename_vehicle', {
							...
							if string.len(data3.value) >= 1 then
								...
								ESX.ShowNotification("El nombre del vehiculo no puede estar vacio")
								menu3.close()
							end
...
function StockVehicleMenu(KindOfVehicle, garage, vehicle_type)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerServerEvent("esx_eden_garage:MoveGarage", trailerProps.plate, garage)
						TriggerEvent('esx:showNotification', 'Tu remolque esta en el garaje')
					else
						TriggerEvent('esx:showNotification', 'No puedes guardar este vehiculo')
					end
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerServerEvent("esx_eden_garage:MoveGarage", vehicleProps.plate, garage)
						TriggerEvent('esx:showNotification', 'Has guardado el vehiculo en el garaje')
					else
						TriggerEvent('esx:showNotification', 'No puedes guardar este vehiculo')
					end
			...
			TriggerEvent('esx:showNotification', 'No estas conduciendo el vehiculo')
		end
	else
		TriggerEvent('esx:showNotification', 'No hay ningun vehiculo para guardar')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El trailer esta de vuelta en el garaje')
					else
						TriggerEvent('esx:showNotification', 'No puedes guardar un trailer aqui')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'El vehiculo se ha develto al garaje')
					else
						TriggerEvent('esx:showNotification', 'No puedes guardar el vehiculo aqui')
					end
				end,vehicleProps)
			end
		else
			TriggerEvent('esx:showNotification', 'No eres el conductor del vehiculo')
		end
	else
		TriggerEvent('esx:showNotification', 'No hay vehiculo para guardar')
	end
end
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if v.fourrieremecano then
					vehicleLabel = vehicleName..': En Deposito'
					table.insert(elements, {label = vehicleLabel, action = 'fourrieremecano'})
				else
					vehicleLabel = vehicleName..': Fuera'
					table.insert(elements, {
						label = vehicleLabel,
						plate = vehicleProps.plate,
						action = 'store'
			...
			table.insert(elements, {label = "No hay ningun vehiculo para recuperar/sacar", action = 'nothing'})
		end
		...
		function(data, menu)
			...
			if data.current.action == 'fourrieremecano' then
				ESX.ShowNotification("Ve a comisaria para recuperar el vehiculo imputado")
			elseif data.current.action ~= nil then
				...
				if not doesVehicleExist then
					...
					ESX.ShowNotification("Ya has recuperado el vehiculo antes")
				end				
			end
		end,
		function(data, menu)
