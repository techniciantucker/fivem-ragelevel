			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_srpfacturas:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
RegisterNetEvent("spainrp_ruleta:darcoche")
AddEventHandler("spainrp_ruleta:darcoche", function() 
    
    ESX.Game.SpawnVehicle("911tbs", { x = 933.29,y = -2.82, z = 78.76 }, 144.6, function (vehicle)
        TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
        ...
        ESX.ShowNotification("Felicidades, ¡Has ganado un Porsche 911!")
    end)
