	...
	while true do
		...
		if onMission then		
			if CanPedHearPlayer(PlayerId(), peds[1]) then
				ShowMPMessage("~r~Robo fallido", "Has despertardo al propietario.", 3500)
				TriggerServerEvent("burglary:ended", true, true, lastDoor, GetStreet(door.x, door.y, door.z))
		...
		if IsPedCuffed(PlayerPedId()) and onMission then
			ShowMPMessage("~r~Robo fallido", "Has sido arrestado.", 3500)
...
AddEventHandler("burglary:finished", function(sum)
	ShowMPMessage("~g~Has realizado el robo con exito", "Has vendido todos los objetos por un valor de" .. sum .. '€', 3500)
end)