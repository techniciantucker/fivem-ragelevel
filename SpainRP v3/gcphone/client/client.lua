...
function ShowNoPhoneWarning () 
  ...
  ESX.ShowNotification("No tienes un ~r~telefono~s~, compralo en la tienda")
end
...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentString("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ Utilizar telefono')
  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= soundDistanceMax) then
          ...
          if (dist <= 1.5) then 
            SetTextComponentFormat("STRING")
            AddTextComponentString("~INPUT_PICKUP~ Contestar")
