      ...
      setTimeout(function(){
       // coin_sound.currentTime = 0;
        ...
        if (game_state.win == game_state.paid) {
		  $.post('http://spainrp_tragaperras/payforplayer', JSON.stringify(game_state.win));
...
function post(name, data) {
    $.post('http://spainrp_tragaperras/'+name, JSON.stringify(data));
}
...
$.post('http://spainrp_tragaperras/playerpays', JSON.stringify(playerbet));
...
var render = function () {
 ...
 // ctx.fillText(game_state.win, 265, 358);
  ctx.fillText("Ganancia " + game_state.paid+"€", 385, 367);
  ...
  // draw game state highlight tiles
	...
	ctx.textAlign = "right";
	ctx.textBaseline = "top";
	  ctx.fillText("Girar gratuitamente " +freespins, 395, 367);
  ctx.fillText("Ganancia " + game_state.paid+"$", 625, 367);
  }
