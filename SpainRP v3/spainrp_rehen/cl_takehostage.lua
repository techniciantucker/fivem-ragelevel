...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola con balas para apuntar a alguien")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("No hay un jugador cerca")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Presiona [G] para soltar, [H] para matar")
...
function releaseHostage()
	...
	if closestPlayer ~= nil then
		...
		print("[SpainRP] No hay jugadores cerca")
	end
end 
