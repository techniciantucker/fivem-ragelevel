...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('esx_srpbuceador:GiveItem')
		StartNPCJob()
		Done = true
	end
end
		...
		if NPCTargetPool ~= nil then
			...
			if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 3 then
				...
				if IsControlJustReleased(1, Keys["E"]) and ESX.PlayerData.job ~= nil then
					TriggerServerEvent('esx_srpbuceador:askStress')
					Wait(200)
					if stressValue < 750000 then
						TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_WELDING', 0, true)
						TriggerServerEvent('esx_srpbuceador:addStress')
						Citizen.Wait(17000)
						StopNPCJob()
						Citizen.Wait(3000)
						...
						ESX.ShowNotification('Estas demasiado cansado, vuelve a la central')
						StopNPCJob(true)
						RemoveBlip(Blips['NPCTargetPool'])
						Onjob = false
					end
...
AddEventHandler('esx_srpbuceador:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_srpbuceador:hasExitedMarker', function(zone)
	if zone == 'Vente' then
		TriggerServerEvent('esx_srpbuceador:stopVente')
	end
...
function CreateBlip()
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == Config.JobName then
		...
		if BlipCloakVente == nil and OnJob == true then

			BlipCloakVente = AddBlipForCoord(Config.Zones.Vente.Pos.x, Config.Zones.Vente.Pos.y, Config.Zones.Vente.Pos.z)
			SetBlipSprite(BlipCloakVente, Config.Zones.Vente.BlipSprite)
			SetBlipColour(BlipCloakVente, Config.Zones.Vente.BlipColor)
			SetBlipAsShortRange(BlipCloakVente, true)
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString(Config.Zones.Vente.BlipName)
			EndTextCommandSetBlipName(BlipCloakVente)
		...
		if BlipCloakVente ~= nil then
			RemoveBlip(BlipCloakVente)
			BlipCloakVente = nil
		end
	...
	while true do
		...
		if ESX.PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_srpbuceador:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_srpbuceador:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and ESX.PlayerData.job ~= nil then
				...
				if ESX.PlayerData.job.name == Config.JobName then
					...
					elseif CurrentAction == 'vente' then
						...
						TriggerServerEvent('esx_srpbuceador:startVente')
...
RegisterNetEvent('esx_srpbuceador:returnStress')
AddEventHandler('esx_srpbuceador:returnStress', function(stress)
	stressValue = stress
end)