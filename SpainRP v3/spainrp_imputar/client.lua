...
--     AddTextComponentString("Deposito Municipal")
...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	_ESX.ShowNotification("El vehiculo con matricula: " .. data.plate .. " ha sido imputado")
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	_ESX.ShowNotification("No tienes suficiente dinero para pagar la multa");
end)
...
function ShowImpoundMenu (action)
	...
	if (IsPedInAnyVehicle(GetPlayerPed(PlayerId()))) then
		_ESX.ShowNotification("Sal del vehiculo para imputar")
		return
	end
	...
	if (vehicle ~= nil) then
		...
		if(_VehicleAndOwner == nil) then
			_ESX.ShowNotification('El dueño del vehiculo es desconocido, no puedes imputarlo');
			return
		end
		...
		_ESX.ShowNotification('No hay vehiculos cercanos');
	end
...
RegisterNUICallback('impound', function(data, cb)
	...
	if (veh.plate:gsub("%s+", "") ~= data.plate:gsub("%s+", "")) then
		_ESX.ShowNotification("El vehiculo procesado y el vehiculo cercano no concuerdan");
		return
	end
...
RegisterNUICallback('unimpound', function(plate, cb)
	Citizen.Trace("Sacando del deposito:" .. plate)
	...
	while true do
		...
		if(_DependenciesLoaded) then
			...
			if (GetDistanceBetweenCoords(_Impound.RetrieveLocation.X, _Impound.RetrieveLocation.Y, _Impound.RetrieveLocation.Z,
				PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z, false) < 2) then
				...
				if (_CurrentAction ~= "retrieve") then
					...
					_ESX.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para sacar el vehiculo del deposito");
			...
			elseif (GetDistanceBetweenCoords(_Impound.StoreLocation.X, _Impound.StoreLocation.Y, _Impound.StoreLocation.Z,
				...
				if (_CurrentAction ~= "store" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "mechanic")) then
					...
					_ESX.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para imputar el vehiculo");
				...
				for i, location in ipairs(_Impound.AdminTerminalLocations) do
					...
					PlayerPedCoords.x, PlayerPedCoords.y, PlayerPedCoords.z, false) < 2) then
						...
						if (_CurrentAction ~= "admin" and (_XPlayer.job.name == "police" or _XPlayer.job.name == "mechanic")) then
							...
							_ESX.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para ver la lista de vehiculos en el deposito");
						end
