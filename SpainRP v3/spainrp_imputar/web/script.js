$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://spainrp_imputar/escape', JSON.stringify({}));
		}
	};
	...
	function setupImpoundForm(data) {
		...
		$('#reason').attr("placeholder", `Introduce un motivo de al menos ${rules.minReasonLength} caracteres o mas`)
	}
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://spainrp_imputar/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	function setupRetrievalForm(data) {
		...
		if(rData.vehicles[0].hold_o) {
			$(holdBy).text(`Este vehiculo debe ser retirado con una autorizacion de un policia`);
		} else if (rData.vehicles[0].hold_m) {
			$(holdBy).text(`Este vehiculo debe ser retirado con una autorizacion de un mecanico`);
		} else {
			$(holdBy).text(``);
		}
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://spainrp_imputar/unimpound', JSON.stringify(plate));
	});

	$('table').on('click', '.unlock', function () {
		var plate = $(this).parent().parent().find('#plate').text();
		$.post('http://spainrp_imputar/unlock', JSON.stringify(plate));
	});
	...
	$('#cancel, #exit').click(function (event) {
		$.post('http://spainrp_imputar/escape', null);
	});
