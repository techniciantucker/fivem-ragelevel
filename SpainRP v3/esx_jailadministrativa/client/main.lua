...
RegisterNetEvent('esx_jailpolicialadministrativa:jail')
AddEventHandler('esx_jailpolicialadministrativa:jail', function(jailTime)
		...
		Citizen.CreateThread(function()
			...
			while JailTime > 0 and not unjail do
				...
				if JailTime % 120 == 0 then
					TriggerServerEvent('esx_jailpolicialadministrativa:updateRemaining', JailTime)
				end
			...
			-- jail time served
			TriggerServerEvent('esx_jailpolicialadministrativa:unjailTime', -1)
...
Citizen.CreateThread(function()
	while true do
		...
		if JailTime > 0 and IsJailed then
			...
			draw2dText(_U('remaining_msg', ESX.Math.Round(fastTimer)), { 0.355, 0.900 } )
...
RegisterNetEvent('esx_jailpolicialadministrativa:unjail')
AddEventHandler('esx_jailpolicialadministrativa:unjail', function(source)
	unjail = true
	JailTime = 0
	fastTimer = 0
end)
...
AddEventHandler('playerSpawned', function(spawn)
	if IsJailed then
		...
		TriggerServerEvent('esx_jailpolicialadministrativa:checkJail')
	end
end)
