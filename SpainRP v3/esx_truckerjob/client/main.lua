...
function MenuVehicleSpawner()
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			TriggerServerEvent('esx_srpcamionero:askStress')
			Wait(100)
			if stressValue < 750000 then

				TriggerServerEvent('esx_srpcamionero:addStress')
...
AddEventHandler('esx_srpcamionero:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_srpcamionero:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
--[[ 			ESX.ShowNotification(_U('pay_repair'))
 ...
 ]]			TriggerServerEvent("esx_srpcamionero:pay", amount)
...
RegisterNetEvent('esx_srpcamionero:returnStress')
AddEventHandler('esx_srpcamionero:returnStress', function(stress)
	stressValue = stress
end)
		...
		if PlayerData.job ~= nil then
			if IsJobTrucker() then
				...
				if isInMarker and not hasAlreadyEnteredMarker then
					...
					TriggerEvent('esx_srpcamionero:hasEnteredMarker', currentZone)
				end

				if not isInMarker and hasAlreadyEnteredMarker then
					hasAlreadyEnteredMarker = false
					TriggerEvent('esx_srpcamionero:hasExitedMarker', lastZone)
				end
...
function MissionLivraisonLetsGo()
	...
	BeginTextCommandSetBlipName("STRING")
	SetBlipSprite (Blips['annulermission'], 163)
	SetBlipDisplay(Blips['annulermission'], 4)
	SetBlipScale  (Blips['annulermission'], 1.1)
	SetBlipColour (Blips['annulermission'], 1)
	SetBlipAsShortRange(Blips['annulermission'], true)
...
function MissionLivraisonStopRetourDepot()
	...
	EndTextCommandSetBlipName(Blips['delivery'])
	SetBlipSprite (Blips['delivery'], 357)
	SetBlipDisplay(Blips['delivery'], 4)
	SetBlipScale  (Blips['delivery'], 0.9)
	SetBlipColour (Blips['delivery'], 5)
