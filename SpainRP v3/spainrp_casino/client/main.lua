...
local blips = {
	{title="Casino", colour=4, id=431, x = 930.17, y = 41.8, z = 37.3}
}
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'casino' or platiniumTicket == true then
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then
				...
				if IsControlJustReleased(1, 38) then
					if IsPedInAnyVehicle(player, true) then
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Entrada Gratuita",  value = 'classicTicket'},
							{label = "Entrada Oro",  value = 'goldTicket'},
							{label = "Entrada Platinium", value = "platiniumTicket"}
						}	

						if PlayerData.job and PlayerData.job.name == 'casino' then
							table.insert(elements, {label = "Entrada Trabajador", value = "worker"})
						end
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CasinoCloak', {
							...
							elseif data.current.value == 'worker' then
								if PlayerData.job.name=='casino' then
									...
									menu.close()
									ESX.ShowNotification('~r~No trabajas para el casino')
								end
							end
						...
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Entrada Gratuita "..Config.classicPrice.."€",  value = 'classicTicket'},
							{label = "Entrada Oro "..Config.goldenPrice.."€",  value = 'goldTicket'},
							{label = "Entrada Platinium "..Config.platiniumPrice.."€", value = "platiniumTicket"}	
						}
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='CasinoActions' then
		...
		CurrentActionMsg  = "Presiona ~r~[E]~b~ para administrar el casino"
		CurrentActionData = {}
	elseif zone == "CasinoCloak" then
		CurrentAction     = 'CasinoCloak'
		CurrentActionMsg  = "Presiona ~r~[E]~p~ para abrir el vestuario"
		CurrentActionData = {}
	elseif zone == "CasinoFridge" then
		CurrentAction     = 'CasinoFridge'
		CurrentActionMsg  = "Presiona ~r~[E]~p~ para abrir la nevera"
		CurrentActionData = {}
	elseif zone == "CasinoBar" then
		CurrentAction     = 'CasinoBar'
		CurrentActionMsg  = "Presiona ~r~[E]~p~ para abrir el bar"
		CurrentActionData = {}
...
AddEventHandler('program-casino:openTicketMenuClient', function(worker)
	print(worker)
	local elements = {
		{label = "No monstrar",  value = 'notShow'}
	}
	if goldTicket then
		table.insert(elements, {label = "Mostrar el boleto de oro", value = 'showGolden'})
	end
	if platiniumTicket then
		table.insert(elements, {label = "Monstrar el boleto platinium", value = 'showPlatinium'})
	end
	if classicTicket then
		table.insert(elements, {label = "Monstrar boleto gratis", value = 'showClassic'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pokazTicket', {
		...
		elseif data.current.value == 'showGolden' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene una entrada de oro", worker)
		elseif data.current.value == 'showPlatinium' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene una entrada platinium", worker)
		elseif data.current.value == 'showClassic' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene una entrada gratis", worker)
		end
...
function OpenCasinoActionsMenu()
	PlayerData = ESX.GetPlayerData()
	local elements = {
		{label = "Pedir entrada",  value = 'ticket'}
	}
	if closeStock == true then
		table.insert(elements, {label = "Armario", value = 'weaponArmory'})
	end
	if PlayerData.job.grade_name == 'boss' then
	 	table.insert(elements, {label = "Acciones de jefe", value = 'boss_actions'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'casino_mgmt', {
		...
		if data.current.value == 'ticket' then
			...
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				...
				ESX.ShowNotification('~r~No hay un jugador cerca')
			end
...
function OpenVaultMenu()
	...
	local elements = {
		{label = "Sacar Arma", value = 'get_weapon'},
		{label = "Meter Arma", value = 'put_weapon'},
		{label = "Sacar Objeto", value = 'get_stock'},
		{label = "Meter Objeto", value = 'put_stock'}
	}
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('program-casino:getVaultWeapons', function(weapons)
	  ...
	  ESX.UI.Menu.Open(
		...
		{
		  title    = "Cantidad de balas a sacar",
		  align    = 'top-left',
		  elements = elements,
		},
		function(data, menu)
