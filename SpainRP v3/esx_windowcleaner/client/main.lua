...
function StartNPCJob()
	...
	TriggerServerEvent('esx_srplimpiacristales:askStress')
		Wait(500)
		if stressValue < 750000 then
			...
			Done = true
			TriggerServerEvent('esx_srplimpiacristales:addStress')
		else
...
function StopNPCJob(cancel)
    ...
    if cancel then
		...
		TriggerServerEvent('esx_srplimpiacristales:GiveItem')
		StartNPCJob()
		Done = true
	end
end
...
RegisterNetEvent('esx_srplimpiacristales:returnStress')
AddEventHandler('esx_srplimpiacristales:returnStress', function(stress)
	stressValue = stress
end)
...
AddEventHandler('esx_srplimpiacristales:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_srplimpiacristales:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_srplimpiacristales:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_srplimpiacristales:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_srplimpiacristales:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'Vente' then
						TriggerServerEvent('esx_srplimpiacristales:startVente')
					end
