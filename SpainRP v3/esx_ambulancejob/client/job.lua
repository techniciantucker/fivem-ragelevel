		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = 'Abrir menu ems', value = 'menuambulance'}
				}
			}, function(data, menu)
				...
				if closestPlayer == -1 or closestDistance > 1.0 then
					...
					if data.current.value == 'revive' then
						...
						ESX.TriggerServerCallback('esx_srpems:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									TriggerServerEvent('esx_srpems:removeItem', 'medikit')
									TriggerServerEvent('esx_srpems:revive', GetPlayerServerId(closestPlayer))
						...
						ESX.TriggerServerCallback('esx_srpems:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('esx_srpems:removeItem', 'bandage')
									TriggerServerEvent('esx_srpems:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						ESX.TriggerServerCallback('esx_srpems:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('esx_srpems:removeItem', 'medikit')
									TriggerServerEvent('esx_srpems:heal', GetPlayerServerId(closestPlayer), 'big')
									ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
									IsBusy = false
								else
									ESX.ShowNotification(_U('player_not_conscious'))
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
							...
							if amount == nil or amount < 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									menu.close()
									TriggerServerEvent('esx_srpfacturas:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Medico', amount)
								end
							end
						end, function(data, menu)
							menu.close()
					...
					elseif data.current.value == 'put_in_vehicle' then
                        TriggerServerEvent('esx_srpems:putInVehicle', GetPlayerServerId(closestPlayer))
                    elseif data.current.value == 'menuambulance' then
						ESX.UI.Menu.CloseAll()
						Citizen.Wait(250)
						ExecuteCommand('menuems')
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
			...
			then
				TriggerEvent('esx_srpems:hasExitedMarker', LastHospital, LastPart, LastPartNum)
				hasExited = true
			end

			HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

			TriggerEvent('esx_srpems:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('esx_srpems:hasEnteredMarker', function(hospital, part, partNum)
...
AddEventHandler('esx_srpems:hasExitedMarker', function(hospital, part, partNum)
...
RegisterNetEvent('esx_srpems:putInVehicle')
AddEventHandler('esx_srpems:putInVehicle', function()
	local playerPed = PlayerPedId()
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_srpems:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_srpems:buyJobVehicle', function (bought)
					if bought then
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		TriggerServerEvent('esx_srpems:giveItem', data.current.value)
	end, function(data, menu)
		menu.close()
	end)
end
...
RegisterNetEvent('esx_srpems:heal')
AddEventHandler('esx_srpems:heal', function(healType, quiet)
	local playerPed = PlayerPedId()
