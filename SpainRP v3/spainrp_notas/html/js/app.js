...
document.onkeyup = function (data) {
    if (data.which == 27) { // Escape key
        $.post('http://spainrp_notas/escape', JSON.stringify({}));
        if (notepadreader == true) {
            var x = document.getElementById("p1").value;
            $.post('http://spainrp_notas/updating', JSON.stringify({ text: x}));
            $("#main").fadeOut();
...
function dropNotepad() {
    var x = document.getElementById("p1").value;
    if (x == ""){
        $.post('http://spainrp_notas/droppingEmpty', JSON.stringify({}));
    }else {
        $.post('http://spainrp_notas/dropping', JSON.stringify({ text: x}));
        $("#main").fadeOut();
        document.getElementById("p1").value = "";
        $("#main").css('display', 'none');
    }   
