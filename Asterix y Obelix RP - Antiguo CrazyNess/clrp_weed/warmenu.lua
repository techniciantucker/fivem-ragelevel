...
local function setMenuProperty(id, property, value)
    if id and menus[id] then
        ...
        debugPrint(id..' propiedad del menú cambiado: { '..tostring(property)..', '..tostring(value)..' }')
    end
end
...
function WarMenu.OpenMenu(id)
    if id and menus[id] then
        ...
        debugPrint(tostring(id)..' menú abierto')
    else
        debugPrint('Falló al abrir '..tostring(id)..' menú: no existe')
    end
end
...
function WarMenu.CloseMenu()
    if menus[currentMenu] then
        if menus[currentMenu].aboutToBeClosed then
            ...
            setMenuVisible(currentMenu, false)
            debugPrint(tostring(currentMenu)..' menú cerrado')
            ...
            debugPrint(tostring(currentMenu)..' menú a punto de ser cerrado')
        end
    end
end
    ...
    if menus[currentMenu] then
        ...
        if isCurrent then
            if currentKey == keys.select then
                ...
                debugPrint(buttonText..' botón presionado')
                return true
        ...
        debugPrint('Fallo al crear '..buttonText..' botón: '..tostring(currentMenu)..' menú: no existe')
...
function WarMenu.MenuButton(text, id)
    if menus[id] then
        ...
        debugPrint('Fallo al crear '..tostring(text)..' botón de menú: '..tostring(id)..' el submenú no existe')
    end
...
function WarMenu.CheckBox(text, bool, callback)
    ...
    if WarMenu.Button(text, checked) then
        bool = not bool
        debugPrint(tostring(text)..' casilla de verificación cambiada a '..tostring(bool))
        callback(bool)
