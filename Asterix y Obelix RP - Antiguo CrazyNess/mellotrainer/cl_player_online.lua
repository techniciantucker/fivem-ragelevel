    ...
    for i=0, 256, 1 do
		if(NetworkIsPlayerConnected(i) and i ~= me) then
            ...
            table.insert(players, {
            	['ped'] = GetPlayerPed(i), 
            	['menuName']= scoreboardID .. ": " .. playerName,
            	['data'] = {
            		['sub'] = "playeroptions",
            		['share'] = scoreboardID
            	},
...
function spectatePlayer(target)
	...
	if(featureSpectate)then
		...
		TriggerServerEvent('mello:logToadmin', plrname .. " is now spectating " .. target['menuName'], "SPEC", "SPECTATE")
		...
		local plrname = GetPlayerName(PlayerId())
		TriggerServerEvent('mello:logToadmin', plrname .. " stopped spectating", "SPEC", "SPECTATE")
...
function drawRoute(target)
	...
	if (featureDrawRoute) then
		if (DoesEntityExist(target['ped'])) then
			...
			local plrname = GetPlayerName(PlayerId())
			TriggerServerEvent('mello:logToadmin', plrname .. " drew route to " .. target['menuName'], "ROUTE", "ROUTE")
...
function teleportToPlayer(target)
	...
	local plrname = GetPlayerName(PlayerId())
	TriggerServerEvent('mello:logToadmin', plrname .. " teleported to " .. target['menuName'], "GOTO", "GOTO")
...
function teleportIntoPlayerVehicle(target)
	...
	local plrname = GetPlayerName(PlayerId())
	TriggerServerEvent('mello:logToadmin', plrname .. " teleported into " .. target['menuName'] .. "'s vehicle", "GOTO", "GOTO")
